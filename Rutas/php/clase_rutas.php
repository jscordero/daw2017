<?php
	...
	class rutas{
		...
		function nuevaRuta($nombre,$kilometros,$minutos,$localidad, $consejos,$dificultad,$valoracion,$pdf,$max_res,$mapa){
			$mensaje="";
		...
		function seleccionarTodasLasRutas(){			
			$consulta="select rutas.ID as ID, rutas.NOMBRE as NOM, rutas.KILOMETROS as KILOMETROS, rutas.MINUTOS as MINUTOS, localidad.LOCALIDAD as LOC, rutas.CONSEJOS as CONSEJOS, dificultadrutas.DIFICUTAD as DIFICULTAD,  CONVERT((select avg(historial.VALOR_RUTA) from historial where historial.ID_RUTA = rutas.id),DECIMAL(4,2)) as VALORACION, rutas.PDF as PDF,  rutas.MAX_RESERVAS as MAX_RESERVAS, rutas.MAPA as MAPA from rutas  join localidad on localidad.ID_LOCALIDAD = rutas.LOCALIDAD  join dificultadrutas on dificultadrutas.ID_DIFICULTAD = rutas.DIFICULTAD order by rutas.id asc";			
		...
		function seleccionarUnaRuta($id){
			$consulta="select rutas.ID as ID, rutas.NOMBRE as NOMBRE, rutas.KILOMETROS as KILOMETROS, rutas.MINUTOS as MINUTOS, localidad.LOCALIDAD as LOCALIDAD, rutas.CONSEJOS as CONSEJOS, dificultadrutas.DIFICUTAD as DIFICULTAD,  CONVERT((select avg(historial.VALOR_RUTA) from historial where historial.ID_RUTA = rutas.id),DECIMAL(4,2))  as VALORACION, rutas.PDF as PDF,  rutas.MAX_RESERVAS as MAX_RESERVAS, rutas.MAPA as MAPA from rutas  join localidad on localidad.ID_LOCALIDAD = rutas.LOCALIDAD  join dificultadrutas on dificultadrutas.ID_DIFICULTAD = rutas.DIFICULTAD  where id=".$id;
			if($resultado=$this->conexion->query($consulta)){
				return $resultado;
			}
		}
